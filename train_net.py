...
def train():
    with tf.Graph().as_default():
        ...
        # in_data: 语谱图 Y_data: 标签 VAD_data:静音段
        ...
        for step in range(init_step, init_step + max_steps):
            ...
            in_data_np = np.concatenate(
                [np.reshape(item['Sample'], [1, -1, NEFF])
                 for item in data_batch])
            VAD_data_np = np.concatenate(
                [np.reshape(item['VAD'], [1, -1, NEFF])
                 for item in data_batch])
            ...
            #     for item in data_batch])
            Y_data_np = np.concatenate(
                [np.reshape(item['Target'], [1, -1, NEFF, 2])
                 for item in data_batch])
            ...
            if last_epoch != data_generator.epoch:
                ...
                # "val_loss" change into "loss_value"？？？？？？？？
