...
def stft(sig, frameSize, overlapFac=0.75, window=np.hanning):
    ...
    #print(type(samples.strides))
    tmp = samples.strides[0]
    #print(cols)

    frames = stride_tricks.as_strided(
        samples,
        shape=(cols, frameSize),
        strides=(tmp * hopSize, tmp)).copy()
    frames *= win
    #print(frames.shape)
    #print(type(frames))
...
class DataGenerator(object):
    ...
    def make_noise(self):
        speaker_num = len(list(self.speaker_file.items()))
        mix_wav_num = np.random.randint(5, 10)
        mixed = None
        for mix in range(mix_wav_num):
            num = np.random.randint(0, speaker_num-1)
            audios = list(list(self.speaker_file.items())[num])[1]
            nums = len(audios)
            ids = np.random.randint(0, nums-1)
            speech, _ = librosa.load(audios[ids])
            if mixed is None:
                mixed = speech
            else:
                while len(speech) < len(mixed):
                    speech = np.concatenate([speech, speech])
                    speech = np.reshape(speech, (-1,))
                mixed += speech[:len(mixed)]
        return mixed
    ...
    def reinit(self):
        ...
        for spkr in self.speaker_file.values():
            for i in spkr:
                speech_2 = self.make_noise()
                speech_1, _ = librosa.core.load(i, sr=SAMPLING_RATE)
                while len(speech_1) < 25600:
                    speech_1 = np.concatenate([speech_1, speech_1])
                while len(speech_2) < len(speech_1):
                    speech_2 = np.concatenate([speech_2, speech_2])
                speech_2 = speech_2[:len(speech_1)]
                ...
                speech_1_tmp = stft(speech_1, FRAME_SIZE)
                speech_1_spec = np.abs(speech_1_tmp[:, :NEFF])
            ...
            pickle.dump(self.samples, open('val.pkl', 'wb'))
            self.total_sample = len(self.samples)
